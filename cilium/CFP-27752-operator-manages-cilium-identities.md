...
Safe upgrade, without downtime, requires simultaneous CID management compatibility. The CID controller in Operator is designed to support simultaneous CID management and won’t interfere with Agents.

The user needs to update Operator first to enable Operator managing CIDs, before Agents are updated to stop managing CIDs (the same flag `operator-manages-identities`)
...
Downgrade without downtime is possible only if Agent daemonset is downgraded first -- restart after configuration change, or updated to an earlier version where Agent manages CIDs. Otherwise, if Operator is downgraded first, there will be a downtime lasting at least the amount of time needed for Agent daemonset to be downgraded after the Operator downgrade.
