...
SPIFFE identity registration must occur out-of-band from when the actual SPIFFE identity usage is made via the workload API. In order to register the SPIFFE identities, the Cilium Operator must share access to an admin domain socket with the SPIRE Server, and use that domain socket for creation of new SPIFFE identities in the SPIRE server.

Notably, this means that _one_ Pod out of the Operator deployment must be running on the _same node_ as the SPIRE Server's StatefulSet Pod. This will need Pod Affinity, and some way for the Operator to check if the domain socket is available. This process will also need to run independently of the usual Kubernetes Leadership, unfortunately.
...
* Node with endpoint must clear out Auth table to remove any state that implies authentication occurred for connections with this endpoint (This should already exist)
...
* At this stage, it seems likely that deletion of expired entries will still be done by userspace, although this could change in the future.
...
That is, there are four scenarios:
...
The current design relies primarily upon the datapath containing a cache of authenticated connections, and the datapath falls back to requesting authentication from the userspace agent when the cache does not indicate a connection is authenticated. This introduces a hard dependency on the availability of the agent for _new_ identity->identity pairings. Note that because this will be done on an _identity_ basis rather than an _IP Address_ basis, multiple connections between the same Pods will _not_ need to pay the TLS session handshake time cost. (Usually on the order of up to 100ms, but it adds up.)
