...
* **SIG:** SIG-Observability, SIG-Datapath, SIG-Hubble  
* **Sharing:** Public  
* **Begin Design Discussion:** Jul 6, 2026
...
* The feature is gated by the `enable-synthetic-flow-generation` flag in the `cilium-config` ConfigMap.
...
* Flowgen uses a **special IPv4 Option Header** to distinguish synthetic flows with real user flows. Operators can customize this IP Option Type via `bpf.monitorTraceIPOptionSyntheticFlows` (default `0x5E` ) to avoid conflicts with other network overlays.

* If `bpf.monitorTraceIPOption` is already set ([public doc](https://docs.cilium.io/en/stable/observability/hubble/ip-packet-tracing/)), FlowGen strictly requires `bpf.monitorTraceIPOption` to not be the same as `bpf.monitorTraceIPOptionSyntheticFlows`.  
  * `bpf.monitorTraceIPOption` is used for tracing real customer flows.

## Section 3: Scheduling Packet Generation

Synthetic flows are generated by a standalone `flowgen-client` process that is managed by the `cilium-agent` co-located (on the same node) as the source endpoint. The system needs to identify the node on which the source endpoint is located, and the corresponding `cilium-agent` responsible for initiating these packets. This process is handled by a cooperative control plane involving the central `cilium-operator` pod and the per-node `cilium-agent` daemonset (specifically, its embedded `flowgen-agent` cell).
...
The `FlowGen` subsystem relies on a node-local agent to initiate synthetic traffic generation. **This is implemented natively within `cilium-agent` as a dedicated Hive named `flowgen-agent`.**

![CFP-46532-flowgen-control-plane](images/CFP-46532-flowgen-control-plane.png)

The `flowgen-agent` Hive cell establishes filtered watches on the kube API server for any event that updates the field selector `CiliumFlowGen.status.SourceNode` to the node name hosting the agent. Only one node (`status.SourceNode`) receives a `CreateEvent` from the kubeAPIserver. The `flowgen-agent` controller cell is responsible for setting up the on-node state required for packet generation and invoking the standalone `flowgen-client`.
...
* `flowgen-agent` sets a watch on the kube API-server for any Delete event of the source pod. This step is to ensure that the flowgen system is able to follow pod reschedules/restarts. Pod delete is the only way for a pod’s IP or host node to be updated.

## Section 4: Initiating Synthetic Flow Generation

#### How are synthetic flows distinguished from real flows?

<a id="synthetic-flow-identifier"></a>Synthetic flows generated by the flowgen subsystem contain **one additional IPv4 Options Header** for both specifying directionality and tracing:

* **Type**:  `bpf.monitorTraceIPOptionSyntheticFlows` (default: `0x5E / 94`) \- This is reserved for **RFC3692**\-style experiments.  
  * Presence of the IP option type indicates that the packet is a flowgen-generated packet.  
  * **Length**: `1` byte for Type \+ `1` byte for Length \+ `8` bytes for Value. (Total `10` bytes)  
  * **Value**:	A `64`-bit field composed of  
  * **LSB (Direction Bit):** Indicates that this is a synthetic packet generated by the flowgen subsystem. We intend to reserve this bit for future use cases pertaining to bidirectional synthetic flow generation.   
    * `0` signifies a forward packet, and `1` signifies a reply packet.  
  * **Remaining 62 bits**: Reserved for the **TraceID**. This **TraceID** uniquely identifies the flow across the cluster and is directly observable by the Hubble sub-system.

#### How are synthetic flows generated?

The initiation and transmission of synthetic packets must be carefully integrated into the node-agent architecture to prevent resource exhaustion and ensure strict network namespace isolation. Depending on the level of desired resilience and system constraints, two primary deployment patterns are proposed for the `flowgen-client`:

##### Option 1: Standalone Isolated Process (Preferred)

To ensure that high-throughput packet generation does not starve the core `cilium-agent` threads, the `flowgen-client` is packaged in the node image but executed as a completely decoupled **standalone process**. `cilium-agent` dynamically mounts an isolated Linux network namespace, provisions a veth interface, and launches the client binary into this namespace using `os/exec`—managing it natively as a local datapath Endpoint (analogous to the `cilium-health-responder` paradigm).

Under this model, when the `flowgen-agent` Hive Cell receives a k8s API update, it synchronizes the BPF connection tracking maps and subsequently commands the standalone `flowgen-client` endpoint via a local, lightweight IPC channel (e.g., HTTP or gRPC) to commence flow generation.

**Pros:**

* Memory leaks, runaway file descriptor allocations, or unrecovered panics in the `flowgen-client` will solely crash the client. The core `cilium-agent` process—which is responsible for routing, security, and K8s syncs across the node—remains entirely unaffected.  
* By spawning a native OS process, `cilium-agent` can assign restrictive `cgroups` and lower CPU `nice` priorities to the client, guaranteeing that heavy synthetic tests can never preempt the kernel from executing real networking tasks.  
* Executing the binary natively within a tightly bounded network namespace cleanly avoids the complexities and multi-threading risks associated with attempting to dynamically enter/exit Linux namespaces (`setns()`) via Go goroutines.

**Cons:**

* **IPC Complexity**: Requires defining and maintaining an internal communication interface (RPC/HTTP) strictly bound to localhost for the agent to negotiate tests with the client.  
* **Packaging and Lifecycle Overhead**: Necessitates shipping a secondary independent executable within the `cilium-agent` container image and explicitly scripting its lifecycle (start/stop/kill actions) and health checks inside the agent's Go logic.

##### Option 2: Embedded Hive Cell (In-Band)

In this approach, the `flowgen-client` logic is directly compiled into `cilium-agent` and spun up as an embedded Hive Cell background job using `job.Group`. When the controller detects a new synthetic flow request, it simply executes a native Go function call to a worker goroutine to begin the socket writes, entirely bypassing the need for separate executables or external communication.

**Pros:**

* **Operational Simplicity**: Eliminates IPC mechanisms. The control-plane reconciliation loops can seamlessly communicate with the datapath generator using shared Go channels and synchronized struct states.  
* **Simplified Delivery**: Decreases container orchestration complexity—`cilium-agent` remains a monolithic, uniformly deployable binary with no subprocess dependencies or child process lifecycle tracking overhead.

**Cons:**

* **Goroutine Starvation**: Pumping synthetic packets demands tight loops of blocking `sendto()` syscalls. Due to Go's M:N scheduler, high-rate packet injection can rapidly hoard OS threads, directly starving critical control-plane goroutines in `cilium-agent` (such as endpoint regeneration and BPF map syncs) leading to cascading node issues.  
* **Shared Process Limits**: A memory leak or file descriptor exhaustion (e.g., thousands of leaked test sockets) in the embedded client will trigger the OOM killer against `cilium-agent` globally. This completely eliminates node networking resilience in the event of a failing synthetic test.  
* **Host Contamination Risk**: Because the agent operates inherently in the root network namespace, any bugs in the socket binding logic (e.g., failure in explicitly applying `SO_BINDTODEVICE`) could dangerously leak un-isolated test packets directly onto the host's actual interfaces.

##### Payload Construction (Across both approaches)

To physically generate synthetic packets, the client (whether a separate process or an embedded goroutine) relies on standard Linux sockets. Since the client is written in Go, it leverages `net.Dialer.Control` to access the raw socket file descriptor immediately after creation but before the `connect()` or `bind()` system calls are evaluated by the kernel. The `flowgen-client` subsequently targets this descriptor with `setsockopt(fd, IPPROTO_IP, IP_OPTIONS, ...)` to inject the requisite flowgen-specific IPv4 Option Headers into the outgoing traffic.

The `flowgen-client` would be architected in such a way that it doesn’t expect return packets for any protocol (since it can only generate unidirectional synthetic probes). For an example,
...
During agent startup, the `flowgen-agent` cell residing in `cilium-agent` performs several initialization steps to setup the state:

* Creates a dummy interface named `cilium_flowgen`. This is the interface through which all flows initiated by the standalone `flowgen-client` process are routed.  
...
Adds the requisite on-node kernel routes needed to route the packet from the `flowgen-client` application to the `cilium_flowgen` interface ([more details](#packet-routing)).

### Section 5.2: Packet Routing from flowgen application to `cilium_flowgen` interface

<a id="packet-routing"></a>

Because the `flowgen-client` operates as a decoupled standalone process managed by `cilium-agent`, we must strictly control the packet's routing. We need the packet to egress through the dummy `cilium_flowgen` interface for the [synthetic packet modification and redirection](#packet-modification-and-redirection) to work.
...
Packets that egress the `flowgen-client` endpoint would be intercepted by the `cil_from_flowgen` BPF program attached to the egress hook of the `cilium_flowgen` interface qdisc. The primary job of `cil_from_flowgen` is to modify the packet headers (based on BPF map state), fetch the interface to which the packet needs to be redirected to, and redirect the packets to the retrieved interface.

<a id="cil-from-flowgen"></a>`cil_from_flowgen` will, in order:

* **Lookup** [IPv4 Option Type](#synthetic-flow-identifier) `bpf.monitorTraceIPOptionSyntheticFlows` (default `0x5E`) in pkt IPv4 headers. If present, it is an indication that this is a flowgen-generated synthetic packet.  
  * If not present, drop the packet.  
  * Parse the 32-bit Value field to extract the **TraceID** (from the relevant 30 bits) and store it as `flowgen_trace_id`.  
...
* Retrieve source endpoint info from `ENDPOINTS_MAP` BPF Map, using `__lookup_ip4_endpoint(ip)`. Let's call the source endpoint as `sep`.  
...
* Modify the packet source and destination IPs and ports to the user 4-tuple, using `ctx_store_bytes`.  
...
* Packet hits the `cil_from_container` of the user source pod LXC interface in the host network namespace.

The packet looks exactly as if it originated from within the source pod network namespace. From hereon, the packet traverses the usual DPv2 stack like any other normal packet.
...
* **Lookup** [IPv4 Option Type](#synthetic-flow-identifier) `bpf.monitorTraceIPOptionSyntheticFlows` (default `0x5E`) in pkt IPv4 headers.  
* If found, it redirects the packet onto the ingress hook of the interface to which the `flowgen-client` standalone endpoint process is bound. This will be a macro value (`FLOWGEN_IFINDEX`) populated by the Cilium datapath loader when it creates the interface and loads the TC BPF programs.
...
When a `CiliumFlowGen` CR is created with a `spec.DestinationEndpoint.IP` corresponding to a LoadBalancer VIP (and the user wants to simulate external ingress), the central controller needs an execution origin.
...
* `flowgen-client` on `nodeA` initiates a packet destined for the ELB VIP. It generates the packet with the correct IPv4 option headers.  
...
## Section 7: End-to-End Synthetic Trace Observability Pipeline

> How do we enable customers to trace synthetic flows on-demand  without polluting or being bottlenecked by the primary datapath ring buffers used for tracing live customer flows?

To ensure total isolation between tracing customer traffic and synthetic probing telemetry, we propose to build a parallel synthetic flow observability pipeline, that would operate on completely independent buffers from kernel-space up to the Hubble gRPC API. We also propose tracing of these synthetic flows via a hubble CLI flag: `--synthetic-ip-trace-id`.

**Optimization**: If we intentionally restrict synthetic observability to ***Follow Mode*** *only* (streaming live events, no historical lookback), we can also entirely bypass Hubble's user-space ring buffer.

Here are the required changes across the stack to achieve this isolated pipeline:

#### 1\. Kernel-Space: Dedicated eBPF Perf Ring Buffer

When a packet is marked with the synthetic flow IPv4 option (`monitorTraceIPOptionSyntheticFlows`), its trace events must not be dropped due to congestion in the main `EVENTS_MAP` perf ring buffer.

* **Create `SYNTHETIC_EVENTS_MAP`:** Define a new, dedicated BPF perf event array exclusively for synthetic flows.  
* **Datapath Redirection (`bpf/lib/`):** Update the `ctx_record_trace` and drop notification macros. When the datapath decodes the packet and identifies `bpf.monitorTraceIPOptionSyntheticFlows`, it pushes the `trace_notify` / `drop_notify` C-struct directly to `SYNTHETIC_EVENTS_MAP` instead of the default `EVENTS_MAP`.  
* **Aggregation**: We will keep the default aggregation to **0**, since we intend to capture all traces emitting from synthetic flow processing.  
* **Isolating Trace IDs:** Add a `__u32 synthetic_trace_id` field to the BPF `trace_notify` struct to explicitly pass the trace ID and direction bit.

#### 2\. User-Space: Dedicated Monitor Channels and Parser (`pkg/monitor/`)

The `cilium-agent` must read and parse these events without cross-contaminating the primary gochannels.

* Update the Go `TraceNotify` and `DropNotify` structs in `pkg/monitor/api/types.go` to match the new BPF structs.  
* Instantiate a secondary perf ring buffer reader that polls `SYNTHETIC_EVENTS_MAP`.  
* Send events from this secondary reader into a dedicated Go channel (`syntheticMonitorChan`), entirely separating it from the primary `monitorChan` that feeds standard Hubble parsing.  
* The new parser logic (`decodeSyntheticIpTraceId`) extracts the Trace ID (top 30 bits) and the direction bit (LSB), and constructs a new protobuf:

```protobuf
message SyntheticIPTraceID {
    uint64 trace_id = 1;
    bool   is_reply = 2; 
}
```
...
#### 3\. Hubble Backend: Bypassing the 3rd Ring Buffer (`pkg/hubble/observer/`)

Hubble ordinarily stores all decoded flows in a large user-space `RingBuffer` (the 3rd buffer) to support historical queries (`hubble observe --since 5m`). We will bypass this entirely to prevent synthetic flows from evicting real customer flows out of memory.

* Instead of writing parsed synthetic events to `s.GetRingBuffer().Write(flow)`, the synthetic parser publishes them directly to a live `SyntheticEventBus` (a Go channel broadcaster or simple pub-sub system).  
* When a gRPC `GetFlows` request arrives with the new `--synthetic-ip-trace-id` filter, the `LocalObserverServer` checks if `req.Follow == true`. If the user attempts a historical query, Hubble returns an error: ***"Historical lookback is not supported for synthetic flows. Use follow mode (-f)."***  
* If `req.Follow` is true, `GetFlows` bypasses the standard `RingReader` instantiation. It subscribes directly to the `SyntheticEventBus`, runs the newly implemented `ip_tracing.go` `FilterFunc` against incoming live events, and streams matching flows directly to the gRPC client.

#### 4\. The CLI Interface (`hubble/cmd/observe/`)

Finally, this isolated streaming pipeline is exposed directly to the end customer for on-demand troubleshooting.

* Hook the new CLI argument via `StringSliceVar`: `hubble observe -f --synthetic-ip-trace-id <id>`  
* When specified, the CLI appends `&flowpb.FlowFilter{SyntheticIpTraceId: ids}` to the request whitelist.  
* Update `hubble/pkg/printer/` routines so that synthetic events stand out clearly in the textual CLI output (e.g., embedding `[SyntheticTrace ID: 94, Dir: Fwd]` gracefully in the flow output lines).
...
**Design:** The injection architecture would depend on `cilium-agent` (or a dedicated daemon) running with `hostPID: true` and `securityContext: privileged: true`. The agent would fetch the PID of the target customer pod and use Linux namespace entering capabilities (similar to the `nsenter` command) to bind the client/server sockets directly inside the network stack of the customer's pod namespace.

**Cons with this approach**:

* Binding sockets and generating traffic directly within the user's network namespace hinders the capability to send synthetic flows with the same 5-tuple as an active user flow. For example, we won’t be able to bind a socket on the same port that the server is listening on. This would massively hinder any future design for bidirectional synthetic flow generation.
...
* It would lead to resource exhaustion within the network stack of the endpoint pod in the worst-case scenario (including buffer overflow, port exhaustion, etc.). This can lead to the kernel dropping connections destined towards actual workloads running in the pod.

* Generating traffic directly from the main `cilium-agent` risks processor starvation and resource exhaustion (OOM), destabilizing the entire node. Binding natively inside the customer pod’s network namespace also demands invasive namespace entering (`nsenter`) capabilities which expands the attack surface and violates the principle of least privilege.

#### Alternate Approach 2: In-Band Generation via Ephemeral Containers 

**Design:** This approach involves dynamically injecting ephemeral flowgen containers directly into the user's source and destination pods by appending to the `EphemeralContainers` field in the pod spec.

**Cons with this approach**:

* Running processes in the ephemeral container context could potentially end up consuming the entirety of the pod resource allocation budget, critically impacting the existing user container workloads on the same pod.

* Ephemeral containers, once injected, cannot be deleted. They are meant for basic, user-initiated debugging. They have strict validations, and many fields required for robust client/server applications are incompatible or disallowed compared to regular container specs.
...
Generating synthetic probes from an isolated local namespace (and relying on eBPF to redirect and mutate the packets) is strongly preferred over generating them directly within the customer's pod network namespace.

* Synthetic packets never enter the customer pod's network namespace. The `flowgen-client` and `flowgen-server` standalone processes are completely decoupled from the user's environment, ensuring zero risk of port collision or namespace contamination.
...
* It avoids the necessity of invasive workload privileges like `nsenter`, relying purely on standard eBPF datapath hooks that Cilium already securely manages.

### Key Question: Pod Reschedules

> What happens if the source or destination pods get rescheduled? Would flowgen-agent dynamically re-resolve and pick up the new endpoints?

#### Option 1: Avoid following pods if user has specified pod resource reference as source / destination

The `flowgen-agent` reconciler sets a watch for Delete events on the source or the destination pod endpoint. For an example, if the source pod gets deleted, the `flowgen-agent` cell will:

1. Communicate with the standalone `flowgen-client` process via the local API to stop the generation of packets for the specific traceID.  
2. Delete the entry in the BPF map `flowgen_user_info` keyed by `traceID`.  
3. Set `status.sourceNode` to an empty value.  
4. This signals `cilium-operator` to update the status of the CR to `Finished`, and clean up the watches for this specific CR.

#### Option 2: Make flowgen subsystem resilient to source / destination pod restarts

As mentioned during the create event, the `flowgen-agent` reconciler sets a watch for Delete events on the source pod endpoint. If the source pod gets deleted, the `flowgen-agent` cell will:

1. Communicate with the standalone `flowgen-client` process via the local API to stop the generation of packets for the specific traceID.  
2. Delete the entry in the BPF map `flowgen_user_info` keyed by `traceID`.  
3. Set `status.sourceNode` to an empty value. This serves as a signal to `cilium-operator` to further re-resolve the pods in case the same pod has been rescheduled on a different node.
...
* If the user has specified `spec.SourceEndpoint.Resource`, it signifies that the system needs to retrieve if the same pod has been restarted on some different node. `cilium-operator` tries to re-resolve the endpoint into an IP, with an exponential backoff retry mechanism (k8s default). This operation will be performed only until the standard duration of flow generation elapses.

Once `cilium-operator` is able to retrieve the pod endpoint into an IP, the control plane and `flowgen-agent` cell perform the same setup steps as they did during the `CiliumFlowGen` CR creation event.

### Key Question: flowgend crash / restart

> What happens to active synthetic probing sessions if the cilium-agent / flowgen-client pod restarts / crashes in between?

#### Approach 1: Probing stops once the `flowgend` pod gets restarted

When the `cilium-agent` pod starts (or restarts following a crash), the `flowgen-agent` cell lists all `CiliumFlowGen` CRs where `Status.SourceNode` equals its host node and the state is currently `Running`. Because the in-memory client API sockets and timers died with the agent restart, the cell assumes the session is irrevocably interrupted. It patches the status of these CRs to a terminal state (e.g., condition `Active: False, reason: AgentRestarted`).
...
**On-demand User Interface with Hubble Integration**: Create a user interface that can invoke flow generation on-demand and return a set of Hubble traces corresponding to the synthetic flows.
