...
1. **Shell & CLI Interface**: Native Hive shell commands `bpf/stats/report` and `bpf/stats/diff` (accessible via `cilium shell`), with corresponding aliases under `cilium bpf stats` in `cilium-dbg`.
...
1. **On-Demand Benchmarking**: The user runs the stats commands (either via `cilium shell` or the `cilium bpf stats` CLI wrapper) locally within the `cilium-agent` container (typically by using `kubectl exec` to run the command inside the target pod). The CLI queries the local `cilium-agent` daemon's Hive shell over a Unix domain socket to retrieve the statistics.
...
```mermaid
flowchart TD
    ConfigMap["cilium-config ConfigMap<br>(enable-bpf-stats=true)"] -->|"Container Boot"| Agent["cilium-agent"]
...
flowchart TD
    ...
    User["Human Operator"] --->|"1. kubectl exec"| CLI["cilium bpf stats<br>(or cilium shell)"]
...
This proposal introduces native Hive shell commands `bpf/stats/report` and `bpf/stats/diff` (run via `cilium shell`). For ease of use, these are also exposed as aliases under `cilium-dbg`: `cilium bpf stats` (which internally executes them via the Hive shell).
...
When enabled, the daemon calls the `BPF_ENABLE_STATS` syscall on startup and keeps the returned file descriptor open to ensure the kernel continuously accumulates statistics. Users can then safely `kubectl exec` into the running agent to run the stats commands.
...
```bash
# kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium bpf stats report
```

Or using the Hive shell directly:
```bash
# kubectl exec -it -n kube-system ds/cilium -c cilium-agent -- cilium shell
cilium-node-1> bpf/stats/report
...
(cgroup:root)         cil_sock4_connect    CGroupSockAddr          102         2.12 ms      20784 ns
```
...
- `--cgroup` Filter results by BPF programs that are attached to the root cgroup.
...
Metrics are exposed on the native metrics endpoint (`:9962/metrics`). Runtime metrics can be disabled via the config CLI:

```bash
# Disable metrics
cilium config set metrics "-cilium_bpf_stats"
...
5. **Fallback**: BPF programs not found during the interface scan resolve to `(cgroup:root)`. This is because `bpf_sock` programs are the only node-wide programs to be concerned with and most are attached to the root cgroup.

This resolved context is exposed as follows:

#### A. CLI Report Representation
The CLI report (e.g. `cilium bpf stats) displays the resolved context directly in the **Attachment Point** column.
...
```bash
# cilium bpf stats baseline.json pr_build.json
